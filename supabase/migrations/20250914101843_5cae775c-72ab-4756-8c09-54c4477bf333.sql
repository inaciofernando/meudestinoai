...
DROP POLICY IF EXISTS "Public can read trip documents" ON storage.objects;
DROP POLICY IF EXISTS "Authenticated can upload trip documents" ON storage.objects;
DROP POLICY IF EXISTS "Owners can update their trip documents" ON storage.objects;
DROP POLICY IF EXISTS "Owners can delete their trip documents" ON storage.objects;
...
DROP POLICY IF EXISTS "Owners can update trip images" ON storage.objects;
DROP POLICY IF EXISTS "Owners can delete trip images" ON storage.objects;

-- Policies for trip-documents bucket
-- Public read
CREATE POLICY "Public can read trip documents"
ON storage.objects
FOR SELECT
...
CREATE POLICY "Owners can update their trip documents"
ON storage.objects
FOR UPDATE
USING (
  bucket_id = 'trip-documents'
...
CREATE POLICY "Owners can delete their trip documents"
ON storage.objects
FOR DELETE
USING (
  bucket_id = 'trip-documents'
...
CREATE POLICY "Authenticated can upload trip images"
ON storage.objects
FOR INSERT
WITH CHECK (
...
CREATE POLICY "Owners can update trip images"
ON storage.objects
FOR UPDATE
USING (
  bucket_id = 'trip-images'
...
CREATE POLICY "Owners can delete trip images"
ON storage.objects
FOR DELETE
USING (
  bucket_id = 'trip-images'
