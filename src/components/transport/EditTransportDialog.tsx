...
interface EditTransportDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (booking: TransportBooking) => void;
...
export function EditTransportDialog({ isOpen, onClose, onUpdate, booking }: EditTransportDialogProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    // Set initial file states
    if (booking.ticket_file_name && booking.ticket_file_url) {
      setTicketFiles([{
        name: booking.ticket_file_name,
    ...
    if (booking.voucher_file_name && booking.voucher_file_url) {
      setVoucherFiles([{
        name: booking.voucher_file_name,
    ...
    form.reset({
      ...
      departure_location: booking.departure_location || "",
      arrival_location: booking.arrival_location || "",
      departure_date: booking.departure_date ? parseISO(booking.departure_date) : undefined,
      departure_time: booking.departure_time || "",
      arrival_date: booking.arrival_date ? parseISO(booking.arrival_date) : undefined,
      arrival_time: booking.arrival_time || "",
      provider_name: booking.provider_name || "",
      booking_reference: booking.booking_reference || "",
      confirmation_number: booking.confirmation_number || "",
  ...
  const handleSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast({
        title: "Erro",
        description: "Não foi possível atualizar o transporte",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
